...
public class ProFileRestController {
    ...
    @GetMapping("/{proFileNumber}")
    // Resource 는 springframework 표시되는 걸로 import
    public ResponseEntity<Resource> download(@PathVariable("proFileNumber") Long proFileNumber) {
        ProFileDTO file = proFileService.getFileProById(proFileNumber);

        // 문자열은 path로 변환
        // 실제 파일 경로 생성
        Path filePath = Path.of(file.getProFileName()); // 실제 파일이 저장되어 있는 경로
        ...
        // 스프링에서 추천해준 것이 정석
        String encodedFileName = URLEncoder.encode(file.getProFileOriginName(),
